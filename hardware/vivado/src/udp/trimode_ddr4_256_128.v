...
output  fifo_post_vld
   ...
   wire [9:0] 	fifo_pre_rd_count2;
   reg [10:0] 	fifo_pre_rd_count2_wr_side;
...
// count rd_count
wire afifo1_empty, afifo1_rd_en,afifo1_valid;
afifo1 afifo1_i (
  .rst   (fifo_pre_rst),        // input wire rst
  
  .wr_clk(fifo_pre_wr_clk),  // input wire wr_clk
  .din   (1'b0           ),        // input wire [0 : 0] din
  .wr_en (fifo_pre_wen   ),    // input wire wr_en
  .full  (               ),      // output wire full
  
  .rd_clk(mem_clk     ),  // input wire rd_clk
  .rd_en (afifo1_rd_en),    // input wire rd_en
  .dout  (            ),      // output wire [0 : 0] dout
  .empty (afifo1_empty),    // output wire empty
  .valid (afifo1_valid)    // output wire valid
);

assign afifo1_rd_en = ~afifo1_empty;

always @(posedge mem_clk)
      if (fifo_pre_rst)                       fifo_pre_rd_count2_wr_side <= 11'd0; 
      else if(afifo1_valid & ~fifo_pre_rd_en) fifo_pre_rd_count2_wr_side <= fifo_pre_rd_count2_wr_side + 1'd1; 
      else if((~afifo1_valid) & fifo_pre_rd_en) fifo_pre_rd_count2_wr_side <= fifo_pre_rd_count2_wr_side - 4'd2; 
      else if(afifo1_valid & fifo_pre_rd_en) fifo_pre_rd_count2_wr_side <= fifo_pre_rd_count2_wr_side - 4'd1; 
      
assign fifo_pre_rd_count2 = fifo_pre_rd_count2_wr_side[10:1];  
...
//     )deepfifo_inst
     (
      ...
      .fifo_pre_rd_count2		(fifo_pre_rd_count2_wr_side),
...
//	.probe1( fifo_pre_dout[63:0] ), // input wire [31:0]  probe1 
//	.probe2( fifo_pre_empty  ), // input wire [0:0]  probe2 
//	.probe3( fifo_pre_rst  ) 
//);  
