module Top (
  ...
  // pl_clk is the 100 MHz PS PL clock: 1 ms = 100000 cycles, 1 us = 100 cycles.
  ...
  // pl_clk cycle later than the SYNC sent to the slave over Type-C.
  always @(posedge pl_clk or negedge pl_aresetn) begin
    if (!pl_aresetn) begin
      sync_trig_d1 <= 1'b0;
      sync_trig_d2 <= 1'b0;
  ...
  always @(negedge pl_clk or negedge pl_aresetn) begin
    if (!pl_aresetn) begin
      sync_out       <= 1'b0;
      slave_sync_out <= 1'b0;
    end else begin
  ...
  assign TRIG_1 = dac_trigger_start;//PL_SYSREF;//clk_dac2;//PL_CLK_50M;//dac_trigger_start;//trig_1_dac_valid;
