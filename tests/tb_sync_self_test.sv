...
module tb_sync_self_test;
  ...
  sync_trigger_link #(.IS_MASTER(0), .WAIT_CYCLES(3), .HIGH_CYCLES(2)) dut (
      ...
      .role_master(1'b0), .self_sync(self_sync),
  ...
  initial begin
    ...
    if (!sync_seen || !sync_ready) begin
      $display("FAIL: self_test did not open external trigger gate");
      $finish;
    end
    ...
    if (accepted != 1) begin
      $display("FAIL: self_test accepted trigger count=%0d expected 1", accepted);
      $finish;
    end
    $display("PASS: self_test bypass accepts XS19 trigger without XS20 SYNC");
    $finish;
  end
endmodule
