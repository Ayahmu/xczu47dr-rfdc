...
module tb_udp_rvctrl_protocol;
  ...
  initial begin
    ...
    repeat (2) @(negedge clk);
    check_condition(legacy_seen == 1'b0, "bare unknown UDP word must not be forwarded as instruction");
    ...
    send_word(64'd0);
    repeat (4) @(negedge clk);
    check_condition(legacy_seen == 1'b1, "framed instruction packet should reach instruction path");
    check_condition(legacy_data == 64'd0, "framed instruction second word mismatch");
    ...
    $display("PASS: RVCTRL0/RVCTRL1/RFCTRL2 packets route to the PL control path and framed UDP instructions");
    $finish;
  end
endmodule
