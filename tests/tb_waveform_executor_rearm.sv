...
module tb_waveform_executor_rearm;
  localparam [63:0] CH1_ADDR = 64'h0000000000000000;
  localparam [63:0] CH2_ADDR = 64'h0000000000010000;
  ...
  initial begin
    ...
    send_instr({CH2_ADDR, 32'd4096, 32'h00000022});
    send_instr(128'h000000000000000000000000000000f3);
    wait(cmd_count == 2);
    repeat(2) @(negedge clk);

    check_condition(last_cmd_addr == CH2_ADDR, "new frame should replace stale WAITTRIG frame and issue CH2 command");
    $display("PASS: waveform executor accepts a new frame after stale WAITTRIG");
    $finish;
  end
endmodule
