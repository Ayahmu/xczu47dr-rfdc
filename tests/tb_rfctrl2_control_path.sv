...
module tb_rfctrl2_control_path;
  ...
  task send_status_packet;
    begin
      ...
      @(negedge clk);
      udp_tdata = 64'h0000000200000003;
      @(negedge clk);
  ...
  task wait_for_status_response;
    ...
    begin
      timeout_cycles = 0;
      while ((response_count < 19) && (timeout_cycles < 80)) begin
        @(negedge clk);
  ...
  initial begin
    ...
    check_condition(response_count == 19, "STATUS must produce nineteen RFRESP2 words");
    ...
    check_condition(responses[1] == 64'h0000000200000003, "response STATUS header mismatch");
    check_condition(responses[2] == 64'h0000008000510002, "response sequence mismatch");
