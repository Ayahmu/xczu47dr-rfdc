...
module tb_rfctrl2_control_path;
  ...
  task wait_for_status_response;
    ...
    begin
      timeout_cycles = 0;
      while ((response_count < 7) && (timeout_cycles < 64)) begin
        @(negedge clk);
  ...
  initial begin
    ...
    check_condition(response_count == 7, "STATUS must produce seven RFRESP2 words");
    ...
    check_condition(responses[2] == 64'h0000002000510002, "response sequence mismatch");
