...
The user-provided reference project `/home/kyu/workspace/fpga_rfsoc_zjdx_20260503_jiaofu` is also built for `xczu47dr-ffvg1517-2-i`. Its generated RFDC XCI uses DAC tile 2 slices 20/22 with `DAC2_Sampling_Rate=1`, `DAC2_Refclk_Freq=125.000`, and interpolation mode `1`. The custom target currently runs DAC2/DAC3 at 2.4 GS/s with 2x interpolation, uses a 1.2 GS/s host/PL stream, sets the HMC7044 DAC refclk outputs to 120 MHz, and configures DAC3 to use the DAC2/DAC230 clock source rather than its own PLL. Tile 2 DAC20/DAC22 are configured for Nyquist Zone2 so a 400 MHz host tone can be measured as an approximately 2.0 GHz Zone2 image with the 2.4 GS/s DAC rate.

The custom PL includes an HMC7044 sequencer and the firmware waits for its done bit before RFDC startup. The RTL currently drives `RESET_H7044_H_0` low as the released state for the active-high reset net; verify that polarity against the schematic during hardware bring-up. The host waveform default sample rate is 1.2 GS/s to match the custom 2.4 GS/s RFDC configuration with 2x interpolation. The custom firmware no longer initializes PS Ethernet or lwIP.
