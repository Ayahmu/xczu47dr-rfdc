...
def _manual_config(request: WaveformRequest, override: BoardOverride | None) -> gui_model.WaveformConfig:
    ...
    if request.loop:
        sample_rate_hz = float(host.DEFAULT_WAVEFORM_SAMPLE_RATE_HZ)
        rep_hz = 1.0 / record_s
        for item in request.manual_channels:
            enabled = bool(_override_for(override, "channel_enabled", item.channel, item.enabled))
            if not enabled or item.waveform != "iq-sine" or item.frequency_mhz == 0.0:
                continue
            cycles = item.frequency_mhz * 1e6 / rep_hz
            nearest = round(cycles)
            if abs(cycles - nearest) > 1e-9:
                raise ValueError(
                    f"CH{item.channel}: loop sine {item.frequency_mhz:g} MHz is not periodic over "
                    f"{request.record_duration_ns:g} ns. Use a frequency that is an integer multiple of "
                    f"{rep_hz / 1e6:.6f} MHz, or change the record duration."
                )
        if round(record_s * sample_rate_hz) <= 0:
            raise ValueError("loop waveform record duration is too short")
    for item in request.manual_channels:
        enabled = bool(_override_for(override, "channel_enabled", item.channel, item.enabled))
        ...
        duration_s = record_s if request.loop and enabled else item.duration_ns * 1e-9
