  ...
  <div class="page-stack">
    ...
    <section class="work-surface">
      ...
      <el-table :data="filtered" height="calc(100vh - 290px)" @row-click="open">
        ...
        <el-table-column label="模式" width="130"><template #default="{ row }">{{ runModeLabel(row) }}</template></el-table-column>
    ...
    <el-drawer v-model="drawer" title="发波任务详情" size="680px">
      <template v-if="selected">
        ...
        <dl class="detail-grid record-details"><div><dt>目标板卡</dt><dd>{{ boards.byId(selected.board_ids[0])?.name || selected.board_ids[0] }}</dd></div><div><dt>输出模式</dt><dd>{{ runModeLabel(selected) }}</dd></div><div><dt>完成策略</dt><dd>{{ selected.completion_mode }}</dd></div><div><dt>创建</dt><dd>{{ formatTime(selected.created_at) }}</dd></div><div><dt>更新</dt><dd>{{ formatTime(selected.updated_at) }}</dd></div></dl>
