...
<template>
  <div class="page-stack">
    ...
    <el-dialog v-model="uploadOpen" title="上传不可变发布版本" width="680px">
      <el-form label-position="top"><el-form-item label="发布名称"><el-input v-model="uploadForm.label" /></el-form-item><el-form-item label="目标配置"><el-select v-model="uploadForm.target_profile"><el-option label="custom_xczu47dr（单板正常运行）" value="custom_xczu47dr" /><el-option label="custom_xczu47dr_bw（DDR 带宽测试）" value="custom_xczu47dr_bw" /></el-select></el-form-item><el-form-item label="Bitstream (.bit)"><input type="file" accept=".bit" @change="choose($event, 'bit')" /></el-form-item><el-form-item label="Firmware (.elf)"><input type="file" accept=".elf" @change="choose($event, 'elf')" /></el-form-item></el-form>
