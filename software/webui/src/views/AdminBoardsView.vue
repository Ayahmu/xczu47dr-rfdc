...
import type { BoardProfile, InventoryScanResult, SerialPortInfo } from '../types'
...
function resetForm() { Object.assign(form, { name: '', model: 'XCZU47DR RFDC', role: 'master', ip: '192.168.1.128', port: 1234, mac: '', udp_interface: 'enp225s0f0', udp_source_ip: '192.168.1.10', clock_source: 'onboard', target_profile: 'custom_xczu47dr', sync_group: '', jtag_cable_serial: '', serial_path: '', baud_rate: 115200, location: '', notes: '', enabled: true }) }
function edit(board?: BoardProfile) {
  resetForm(); editingId.value = board?.id || ''
  if (board) Object.assign(form, { name: board.name, model: board.model, role: board.role, ip: board.ip, port: board.port, mac: board.mac, udp_interface: board.udp_interface, udp_source_ip: board.udp_source_ip, clock_source: board.clock_source, target_profile: board.target_profile, sync_group: board.sync_group, jtag_cable_serial: board.jtag_cable_serial, serial_path: board.serial_path, baud_rate: board.baud_rate, location: board.location, notes: board.notes, enabled: board.enabled })
  drawer.value = true
}
async function save() {
  try {
  ...
  <div class="page-stack">
      ...
      <el-tabs v-model="activeTab">
        <el-tab-pane label="板卡档案" name="profiles">
          ...
          <el-table :data="filtered" height="calc(100vh - 330px)">
            ...
            <el-table-column label="控制网络" min-width="220"><template #default="{ row }">{{ row.ip }}:{{ row.port }}<small class="table-subtitle">{{ row.udp_interface }} · {{ row.udp_source_ip }}</small></template></el-table-column>
            ...
            <el-table-column label="操作" width="100"><template #default="{ row }"><el-button :icon="Edit3" size="small" @click="edit(row)">编辑</el-button></template></el-table-column>
          </el-table>
        </el-tab-pane>
    ...
    <el-drawer v-model="drawer" :title="editingId ? '编辑板卡档案' : '登记板卡'" size="720px">
      <el-form label-position="top" class="admin-form">
        ...
        <div class="form-grid"><el-form-item label="板卡名称"><el-input v-model="form.name" /></el-form-item><el-form-item label="型号"><el-input v-model="form.model" /></el-form-item><el-form-item label="位置"><el-input v-model="form.location" /></el-form-item><el-form-item label="目标配置"><el-select v-model="form.target_profile"><el-option label="custom_xczu47dr" value="custom_xczu47dr" /><el-option label="custom_xczu47dr_b" value="custom_xczu47dr_b" /><el-option label="custom_xczu47dr_bw" value="custom_xczu47dr_bw" /></el-select></el-form-item></div>
        <div class="form-section-title">控制网络</div>
        <div class="form-grid"><el-form-item label="板卡 IP"><el-input v-model="form.ip" /></el-form-item><el-form-item label="UDP 端口"><el-input-number v-model="form.port" :min="1" :max="65535" controls-position="right" /></el-form-item><el-form-item label="服务器网卡"><el-select v-model="form.udp_interface"><el-option label="enp225s0f0" value="enp225s0f0" /><el-option label="enp225s0f1" value="enp225s0f1" /></el-select></el-form-item><el-form-item label="服务器源 IP"><el-input v-model="form.udp_source_ip" /></el-form-item><el-form-item label="MAC"><el-input v-model="form.mac" /></el-form-item></div>
