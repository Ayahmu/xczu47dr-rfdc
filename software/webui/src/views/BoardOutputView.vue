    ...
    <section class="output-workbench">
      <div class="output-config-column">
        <section class="work-surface compact-surface">
          <div class="config-section-head"><div><h2>输出任务</h2><p>循环缓存长度决定上传到 DDR 的一段正弦；连续输出时长决定多久后静音，0 表示直到手动停止。</p></div><div class="inline-actions"><el-button :icon="RotateCcw" @click="reset">重置</el-button><el-button :icon="Save" @click="drafts.save(boardId)">保存草稿</el-button></div></div>
          <div class="task-form-grid">
            ...
            <label v-else><span>输出保持时间 <small>ms</small></span><el-input-number v-model="draft.outputDurationMs" :min="1" :max="3600000" :step="10" controls-position="right" @change="dirty" /></label>
          </div>
        </section>
