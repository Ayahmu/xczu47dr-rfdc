...
async function send() {
  ...
  try {
    ...
    const record = await runs.create({ jobs: [{ board_id: boardId.value, waveform, override: outputOverride(boardId.value, draft.value), rfdc_config: outputRfdc(boardId.value, draft.value, rfdc.value) }],
      dry_run: false, execution_mode: 'single', completion_mode: 'one_shot', one_shot_duration_ms: draft.value.outputDurationMs })
    currentRunId.value = record.id
    confirmOpen.value = false
    drafts.save(boardId.value)
    ElMessage.success('任务已创建，将自动发送并静音')
...
<template>
  <div class="page-stack">
    ...
    <section class="output-workbench">
      <div class="output-config-column">
        <section class="work-surface compact-surface">
          <div class="config-section-head"><div><h2>输出任务</h2><p>记录长度决定上传样本；循环总时间决定触发后多久自动静音。</p></div><div class="inline-actions"><el-button :icon="RotateCcw" @click="reset">重置</el-button><el-button :icon="Save" @click="drafts.save(boardId)">保存草稿</el-button></div></div>
          <div class="task-form-grid">
            <label><span>任务名称</span><el-input v-model="draft.name" @input="dirty" /></label>
            <label><span>输出模式</span><el-segmented v-model="draft.loop" :options="[{ label: '单次', value: false }, { label: '循环', value: true }]" @change="dirty" /></label>
            <label><span>记录长度 <small>ns</small></span><el-input-number v-model="draft.recordDurationNs" :min="10" :max="1e9" :step="100" controls-position="right" @change="dirty" /></label>
            <label><span>{{ draft.loop ? '循环总时间' : '输出保持时间' }} <small>ms</small></span><el-input-number v-model="draft.outputDurationMs" :min="1" :max="3600000" :step="10" controls-position="right" @change="dirty" /></label>
          </div>
        </section>
      ...
      <aside class="preview-column">
        <section class="work-surface preview-surface">
          <div class="config-section-head"><div><h2>波形预览</h2><p>预览与实际发送使用同一份板卡草稿。</p></div><el-button type="primary" plain :icon="Eye" :loading="previewBusy" @click="generatePreview">生成预览</el-button></div>
    ...
    <section v-if="currentRun" class="run-progress-panel" :class="{ completed: currentRun.state === 'DONE', failed: currentRun.state === 'FAULT' }">
      <div class="run-progress-title">
        <component :is="currentRun.state === 'DONE' ? CheckCircle2 : currentRun.state === 'FAULT' ? AlertTriangle : Radio" :size="24" />
        <div><strong>{{ currentRun.state === 'DONE' ? '发送成功并已自动静音' : currentRun.state === 'FAULT' ? '发送失败' : '正在执行一键发送' }}</strong><span>{{ currentRun.error || stateLabel(currentRun.state) + ' · ' + Math.round(currentRun.progress * 100) + '%' }}</span></div>
    ...
    <div class="fixed-command-bar">
      <div><strong>{{ board?.name }}</strong><span>CH {{ enabledChannels.map(item => item.channel).join(', ') || '-' }} · {{ draft.loop ? '循环 ' + draft.outputDurationMs + ' ms' : '单次 ' + draft.outputDurationMs + ' ms' }}</span></div>
    ...
    <el-dialog v-model="confirmOpen" title="确认发送波形" width="720px">
      <el-alert title="该流程将自动预检、配置 RFDC、上传、触发，并在设定时间后静音。" type="info" show-icon :closable="false" />
      <dl class="send-summary">
        ...
        <div><dt>模式与时长</dt><dd>{{ draft.loop ? '循环输出' : '单次输出' }} · {{ draft.outputDurationMs }} ms 后自动静音</dd></div>
