...
static int Wait_For_Start_Command(void);
...
#define DAC_MTS_PREFERRED_REF_TILE 2U
...
int Configure_DAC_MTS(void)
{
        ...
        u32 EnabledTiles = 0U;
        u32 RefTile = DAC_MTS_PREFERRED_REF_TILE;

        XRFdc_GetIPStatus(&RFdcInst, &IpStatus);
        for (Tile = 0U; Tile < NUM_TILES; Tile++)
        {
                if (IpStatus.DACTileStatus[Tile].IsEnabled == 1)
                        EnabledTiles |= (1U << Tile);
        }

        if (EnabledTiles == 0U)
        {
                xil_printf("ERROR: DAC MTS failed: no enabled DAC tiles.\r\n");
                Publish_DAC_MTS_Status(0U, 1U, 0U);
                return XST_FAILURE;
        }
        ...
        {
                xil_printf("ERROR: DAC MTS init failed, status=0x%08lx ref_tile=%lu\r\n",
                           (unsigned long)Status, (unsigned long)RefTile);
        ...
        xil_printf("Running DAC MTS: tiles=0x%lx reference_tile=%lu\r\n",
                   (unsigned long)DacSyncConfig.Tiles, (unsigned long)RefTile);
        ...
        xil_printf("DAC MTS ready: tiles=0x%lx reference_tile=%lu\r\n",
                   (unsigned long)DacSyncConfig.Tiles, (unsigned long)RefTile);
        return XST_SUCCESS;
}
...
static int Wait_For_Start_Command(void)
{
        ...
        xil_printf("RFDC initialization is paused.\r\n");
        xil_printf("Type S to reinitialize RFDC, run MTS, and reset NCO phase: ");

        for (;;)
        {
                if (XUartPs_IsReceiveData(RFDC_CONSOLE_UART_BASEADDR))
                {
                        StartCommand = (char)XUartPs_RecvByte(RFDC_CONSOLE_UART_BASEADDR);
                        if ((StartCommand == 'S') || (StartCommand == 's'))
                        {
                                xil_printf("%c\r\n", StartCommand);
                                return XST_SUCCESS;
                        }
...
int main(void)
{
	...
	else
	{
        ...
        for (;;)
        {
                if (InitCycle != 0U)
                {
                        if (Wait_For_Start_Command() != XST_SUCCESS)
                                return XST_FAILURE;

                }

                /* Reinitialize RFDC for every power-on cycle or S command. */
