...
For this `xs17-250mhz` branch, XS17 is a 250 MHz reference. HMC7044 PLL1 divides it by 25 to a 10 MHz PFD and then uses N1=10 to lock the 100 MHz VCXO. The PL sequencer programs the 128 MHz DAC reference; firmware polls only the HMC7044 done bit, then continues RFDC/MTS/NCO startup independently of XS20 synchronization. This branch has not been validated with a physical 250 MHz input. The RTL drives `RESET_H7044_H_0` low as the released state for the active-high reset net; verify that polarity on the board during bring-up.
