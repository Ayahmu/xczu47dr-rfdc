# External 250 MHz XS17 Profile

This document describes the `xs17-250mhz` branch configuration for the custom
XCZU47DR board. It is a build- and static-validation configuration only: do
not claim board validation until a physical 250 MHz source has been connected.

## Clock Path

The HMC7044 selects CLKIN1/XS17. PLL1 uses R1=25 and N1=10, so the 250 MHz
input is divided to the 10 MHz PLL1 PFD and locks the on-board 100 MHz VCXO.
...
2. Use a second coherent reference output as the oscilloscope timebase, or
   use a coherent 10 MHz output as the scope reference where required.
...
Sharing coherent reference outputs between the board and the scope
